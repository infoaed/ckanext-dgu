...
CKAN.Dgu = function($, my) {
  ...
  my.addTableRow = function(table) {
      ...
      newRow.find('.needs-datepicker').datepicker($.datepicker.regional[document.documentElement.lang]);
      return newRow;
  };
    ...
    elements
      ...
      .autocomplete({
        ...
        source: function(request, callback) {
          ...
          var url = CKAN.SITE_URL + '/api/action/tag_autocomplete?vocabulary_id=eurovoc&query=' + _realTerm;
          $.getJSON(url, function(data) {
            // data = { ResultSet: { Result: [ {Name: tag} ] } } (Why oh why?)
            var tags = $.map(data.result, function(value, idx) {
              return value;
            });
            callback(
